...
def epsilon_greedy(value, e, seed = None):
    ...
    prob=np.full(value.shape,e/value.shape[0])
    prob[np.argmax(value)] = 1-e+(e/value.shape[0])
    action=np.random.choice(value.shape[0],1,p=prob)
...
def action_evaluation(env, gamma, v):
    ...
    for s in range(nS):
        for a in range(nA):
            ...
            # YOUR CODE STARTS HERE
            state0, action0 = s, a
            out=env.P[state0][action0]
            for i in range(len(out)):
                q[s,a] = q[s,a]+ out[i][0]*(out[i][2] + gamma*v[out[i][1]])
